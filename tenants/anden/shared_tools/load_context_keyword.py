#!/usr/bin/env python3
"""
Load Context Tool - Intelligently load required context for a task.

This tool uses context_map.json to determine what workflows, memory, and tools
are needed for a given task. It helps the AI avoid context fragmentation by
automatically loading related resources.
...
Output JSON:
{
    ...
    "recommendations": [
        "Consider loading 'gmail' workflow for email operations",
        "Recent campaign activity found - 5 pending approvals"
    ]
}
"""
...
def match_task_pattern(task: str, context_map: Dict) -> Optional[tuple]:
    """
    Match task description to a pattern in context_map.
    Returns (pattern_key, confidence_score) or None.
    """
    ...
    for pattern_key, pattern_data in context_map.get("task_patterns", {}).items():
        ...
        for trigger in triggers:
            if trigger.lower() in task_lower:
                matches += 1
                # Longer triggers get higher weight
                score += len(trigger) / 10.0

        # Normalize by number of triggers
        if matches > 0:
            score = score / len(triggers) + (matches / len(triggers))
...
def load_memory_data(memory_keys: List[str]) -> Dict:
    ...
    for key in memory_keys:
        ...
        try:
            ...
            # Silently skip failed memory loads
            pass
...
def main():
    try:
        ...
        if task:
            match_result = match_task_pattern(task, context_map)
            if match_result:
                matched_pattern, confidence = match_result
